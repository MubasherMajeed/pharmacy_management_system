...
router.post("",(req,res,next)=>{
  const VerifiedDocOder = new VerifiedUserOrder({
  ...
  VerifiedDocOder.save().then(createdDocOder=>{
    res.status(201).json({
      message:'Verified User Oder Added Successfully',
...
router.get("/enteredemail/:email",(req,res,next)=>{
  VerifiedUserOrder.find({supplierEmail: req.params.email}).then(documents=>{
    res.status(200).json({
...
router.delete("/:id", (req, res, next) => {
  VerifiedUserOrder.deleteOne({ _id: req.params.id }).then(result => {
    console.log(result);
    res.status(200).json({ message: 'User verified order deleted!' });
  });
});

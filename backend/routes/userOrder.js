...
const VerifiedUserOrder = require("../models/verifiedUserOrder");


router.post("",(req,res,next)=>{
  const docOder = new UserOrder({
...
router.get("",(req,res,next)=>{
  UserOrder.find().then(documents=>{
    res.status(200).json({
      message : 'User oder added sucessfully',
...
router.get("/enteredemail/:email",(req,res,next)=>{
  UserOrder.find({supplierEmail: req.params.email}).then(documents=>{
    res.status(200).json({
      message : 'User verify oder added sucessfully',
      doctorOders :documents
    });
  });
});
...
router.delete("/:id", (req, res, next) => {
  UserOrder.deleteOne({ _id: req.params.id }).then(result => {
    console.log(result);
    res.status(200).json({ message: 'User order deleted!' });
  });
});

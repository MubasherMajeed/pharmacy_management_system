...
router.post("",multer({storage: storage}).single("image"),(req,res,next)=>{
  ...
  supplierInventory.save().then(createdSupplierInventory=>{
    res.status(201).json({
      message:'SupplierInventory Added Successfully',
      supplierInventory: {
        ...createdSupplierInventory,
        id : createdSupplierInventory._id
...
router.put("/:id",multer({storage: storage}).single("image"), (req,res,next)=>{
  ...
  console.log(supplierInventory);
  SupplierInventory.updateOne({_id: req.params.id}, supplierInventory).then(result => {
    console.log(result);
...
router.get("",(req,res,next)=>{
  ...
  const postQuery = SupplierInventory.find();
  ...
  postQuery.then(documents=>{
    res.status(200).json({
      message : 'supplierInventory added sucessfully',
      supplierInventorys :documents
    });
  });
});
...
router.get("/outofstock",(req,res,next)=>{
  const pageSize = +req.query.pagesize;
  const currentPage = +req.query.page;
  const postQuery = SupplierInventory.find({ $expr: { $lte: [ { $toDouble: "$quantity" }, 1.0 ] }});
  if(pageSize && currentPage){
    postQuery
      .skip(pageSize * (currentPage-1))
      .limit(pageSize);
  ...
  postQuery.then(documents=>{
    res.status(200).json({
      message : 'supplierInventory min quanity items obtained  sucessfully',
      supplierInventorys :documents
    });
  });
});
...
router.get("/abouttooutofstock",(req,res,next)=>{
  const pageSize = +req.query.pagesize;
  const currentPage = +req.query.page;
  const postQuery = SupplierInventory.find({$and: [
...
router.get("/getExpired",(req,res,next)=>{
  const pageSize = +req.query.pagesize;
  const currentPage = +req.query.page;
  const postQuery = SupplierInventory.find({expireDate:{$lte:new Date()}});
  if(pageSize && currentPage){
    postQuery
      .skip(pageSize * (currentPage-1))
      .limit(pageSize);
...
router.get("/getAboutToExpire",(req,res,next)=>{
  ...
  const postQuery = SupplierInventory.find({expireDate:{$lte:new Date(date10),$gte:new Date()}});
  if(pageSize && currentPage){
    postQuery
      .skip(pageSize * (currentPage-1))
      .limit(pageSize);
...
router.get("/:id",(req,res,next)=>{
  SupplierInventory.findById(req.params.id).then(supplierInventory =>{
    if(supplierInventory){
      res.status(200).json(supplierInventory);
    }else{
      res.status(200).json({message:'SupplierInventory not found'});
    }
  });
});
...
router.delete("/:id", (req, res, next) => {
  SupplierInventory.deleteOne({ _id: req.params.id }).then(result => {
    console.log(result);
    res.status(200).json({ message: 'SupplierInventory deleted!' });
  });
});

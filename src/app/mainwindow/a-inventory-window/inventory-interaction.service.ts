...
export class InventoryInteractionService {
  ...
  getInventory(itemsPerPage: number , currentPage:number) {
    ...
    this.http.get<{message: string, inventorys: any}>('http://localhost:4000/api/inventory' + queryParams)
  ...
  getOutofStockInventory() {
    ...
    this.http.get<{message: string, inventorys: any}>('http://localhost:4000/api/inventory/outofstock' )
    .pipe(map(inventoryData => {
     return inventoryData.inventorys.map(inventory=>{
       return{
        email: inventory.email,
  ...
  getAboutToOutofStockInventory() {
    ...
    this.http.get<{message: string, inventorys: any}>('http://localhost:4000/api/inventory/abouttooutofstock' )
    .pipe(map(inventoryData => {
     return inventoryData.inventorys.map(inventory=>{
       return{
        email: inventory.email,
  ...
  getExpiredInventory(){
    this.http.get<{message: string, inventorys: any}>('http://localhost:4000/api/inventory/getExpired')
    .pipe(map(inventoryData => {
     return inventoryData.inventorys.map(inventory=>{
       return{
        email: inventory.email,
  ...
  getAboutToExpireInventory(){
    ...
    this.http.get<{message: string, inventorys: any}>('http://localhost:4000/api/inventory/getAboutToExpire')
    .pipe(map(inventoryData => {
     return inventoryData.inventorys.map(inventory=>{
       return{
        email: inventory.email,
  ...
  addInventory( email: string, name: string, quantity: string, batchId: string, expireDate: string, price: string , image: File) {
    ...
    this.http.post<{message: string, inventory: Inventory}>('http://localhost:4000/api/inventory',inventoryData)
  ...
  updateInventory(id: string , email: string ,name: string, quantity: string, batchId: string, expireDate: string, price: string ,image: File | string){
    ...
    this.http
             .put('http://localhost:4000/api/inventory/' + id , inventoryData)
             .subscribe(response => {
  ...
  updateQuantity(id: string ,quantity: number){
    ...
    this.http
             .put('http://localhost:4000/api/inventory/updateQuantity/' + id , inventory)
             .subscribe(response => {
  ...
  deleteInventory(inventoryId: string) {
    this.http.delete('http://localhost:4000/api/inventory/' + inventoryId)
      .subscribe(() =>{

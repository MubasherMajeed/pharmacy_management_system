...
export class SupplierInteractionService {
  ...
  getSupplier() {
    this.http.get<{message: string, suppliers: any}>('http://localhost:4000/api/supplier')
  ...
  addSupplier( supplierID: string, name: string, email: string, contact: string, drugsAvailable: string) {
    ...
    this.http.post<{message: string, supplierId: string}>('http://localhost:4000/api/supplier',supplier)
  ...
  deleteSupplier(supplierId: string) {
    this.http.delete('http://localhost:4000/api/supplier/' + supplierId)
      .subscribe(() => {

...
export class AuthService{
  ...
  login(email: string, password){
    ...
    this.http.post<{token: string, expiresIn: number, role :string , message: string}>("http://localhost:4000/api/user/login",authData)
      .subscribe(response =>{
  ...
  updateUser(id: string ,  name: string, email: string, nic: string, contact: string, password: string, role: string){
    ...
    this.http
             .put('http://localhost:4000/api/user/' +id , user)
             .subscribe(response => {
  ...
  deleteUser(userId: string) {
    this.http.delete('http://localhost:4000/api/user/' +userId)
      .subscribe(() => {

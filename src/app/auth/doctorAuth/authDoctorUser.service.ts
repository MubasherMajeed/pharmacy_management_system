...
export class AuthDoctorUserService {
  ...
  createDoctorUser(name: string , contact: string , docId: string ,email: string ,password: string ){
    ...
    this.http.post("http://localhost:4000/api/doctorUser/doctorSignup",authDoctorData)
      .subscribe(response =>{
  ...
  login(email: string, password){
    ...
    this.http.post<{token: string, expiresIn: number, name:string, contact: string, email:string,docId:string}>("http://localhost:4000/api/doctorUser/doctorLogin",authDoctorData)
      .subscribe(response =>{
    ...
    ('http://localhost:4000/api/doctorUser/shoppingcart/'+email);
  ...
  //   return this.http.get<{userId:string,role:string}>("http://localhost:4000/api/user/profile/"+id);
  //  }
  ...
  getDoctorDatas(id: string){
    ...
    ('http://localhost:4000/api/doctorUser/' +id);
  }

  getDoctorData() {
    this.http.get<{message: string, doctors: any}>('http://localhost:4000/api/doctorUser/getDoctorUserData')
    .pipe(map(docData => {
  ...
  updateDoctor(id: string ,  name: string, email: string, docId: string, contact: string, password: string){
    ...
    this.http
             .put('http://localhost:4000/api/doctorUser/' +id , doctor)
             .subscribe(response => {
  ...
  deleteUser(userId: string) {
    this.http.delete('http://localhost:4000/api/doctorUser/' +userId)
      .subscribe(() => {
...
//   getAll() {
//     return this.http.get<AuthDoctorData[]>('http://localhost:4000/api/doctorUser');
// }
